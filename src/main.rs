...
fn main() -> Result<()> {
    ...
    let db_path = cli.db.as_ref().and_then(|p| p.to_str());
    let db = db::Db::open(db_path).context("Failed to open RTK tracking database")?;
    ...
    let result = app::App::new(db, cli.refresh).run(&mut terminal);
    ratatui::restore();
